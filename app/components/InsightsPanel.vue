...
async function copySummary(): Promise<void> {
  ...
  try {
    ...
    // Fallback: textarea hack for older webview / browsers without clipboard API
  ...
  copyResetTimer = setTimeout(() => { justCopied.value = false; }, 2_000);
}
  ...
  <div class="flex h-full w-full flex-col">
    ...
    <div v-else-if="state === 'generating'" class="flex flex-1 flex-col min-h-0">
      ...
      <section>
        ...
        <template v-if="streamingInsights.summary || streamingInsights.summaryBullets.length > 0">
          ...
          <ul v-if="streamingInsights.summaryBullets.length > 0" class="space-y-1.5 text-sm" :class="streamingInsights.summary ? 'mt-3' : ''">
    ...
    <div v-else-if="(state === 'ready' || state === 'outdated') && insights" class="flex flex-1 flex-col min-h-0">
      ...
      <div class="flex flex-1 min-h-0 flex-col gap-4 overflow-y-auto px-1 pb-3 pr-3">
        ...
        <section>
          ...
          <ul v-if="insights.summaryBullets.length > 0" class="space-y-1.5 text-sm" :class="insights.summary ? 'mt-3' : ''">
    ...
    <Dialog v-model:open="showClearDialog">
      <DialogContent class="max-w-md">
        ...
        <DialogFooter>
          <Button variant="secondary" @click="showClearDialog = false">{{ t('common.cancel') }}</Button>
