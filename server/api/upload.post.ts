...
export default defineEventHandler(async (event) => {
  ...
  // `<cache>/<sha>/waveform.json` so the player opens with peaks already
  // cached. Failure is logged but doesn't fail the upload — /api/waveform
  // stays as the on-demand fallback for the rare miss.
  ...
  return { hash: sha, imported, existed };
});
