...
// Tray icons. macOS auto-tints any image whose filename ends in
// "Template" — so even though our source has color we pass it through
// as a template and accept that menu-bar legibility will be imperfect
// until we hand-author a monochrome silhouette.
const tray1x = await sharp(svg, { density: 96 })
  .resize(22, 22, { fit: 'contain', background: { r: 0, g: 0, b: 0, alpha: 0 } })
  .png().toBuffer();
const tray2x = await sharp(svg, { density: 192 })
