...
把视频拖进来 → 本地 Whisper 转写 → 边播放边按需翻译。Subcast 同时发布 macOS / Windows 桌面版以及开源 Web 版（开发者使用）。
...
<!-- TODO(v0.1.0): 此处插入 Setup Wizard Step 1 截图。 -->
...
Subcast 是 AGPL v3 开源软件：fork 后做任何分发（包括网络服务）都必须以 AGPL v3 开源你的修改。
