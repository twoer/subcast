...
import { checkBundledBinaries, formatMissingForDialog } from './binaryCheck.js';
...
async function bootstrap(): Promise<void> {
  ...
  if (mainWindow) {
    ...
    installAppMenu({
      onQuit: requestQuit,
      // About lives as a Settings tab now (so it shares vue-i18n with the
      ...
      onAbout: () => {
        if (mainWindow && !mainWindow.isDestroyed()) {
          ...
          mainWindow.focus();
          mainWindow.webContents.send('subcast:navigate', '/settings#about');
        }
      },
      onExportDiagnostics: runExportDiagnostics,
      onCheckForUpdates: runCheckForUpdates,
