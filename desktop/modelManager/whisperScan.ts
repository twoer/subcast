  ...
  for (const parent of devParents) {
    ...
    for (const parts of candidates) {
      ...
      if (parts[0]!.includes('*')) {
        ...
        let entries: string[] = [];
        try {
          // readdirSync from sync API to keep this function pure-sync at
          // call site. Failures (missing parent dir) yield empty list.
          // eslint-disable-next-line @typescript-eslint/no-require-imports
          entries = (require('node:fs') as typeof import('node:fs')).readdirSync(parent);
        } catch {
          continue;
        }
